...
export interface CreateGraphConfigDialogData {
  name?: string;
  description?: string;
}
...
@Component({
  standalone: true,
  selector: 'create-graph-config-dialog',
  template: `
    <h2 mat-dialog-title>Create Graph Configuration</h2>
    <mat-dialog-content>
      <mat-form-field appearance="outline" class="full-width">
        <mat-label>Configuration Name</mat-label>
        <input matInput [(ngModel)]="name" placeholder="My Graph Config" required>
      </mat-form-field>
      <mat-form-field appearance="outline" class="full-width">
...
export class CreateGraphConfigDialogComponent {
  name: string = '';
  description: string = '';

  constructor(
    @Inject(MAT_DIALOG_DATA) public data: CreateGraphConfigDialogData | null,
    private dialogRef: MatDialogRef<CreateGraphConfigDialogComponent>
  ) {
    if (data) {
      this.name = data.name || '';
