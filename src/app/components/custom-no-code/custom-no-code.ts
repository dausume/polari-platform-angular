...
import { StateToolItem, BoundClassDefinition, ClassMemberStateRequest, HelperClassDefinition, SubSolutionDefinition } from './editor/state-tool-sidebar/state-tool-sidebar.component';
import { StateSpaceClassRegistry, StateSpaceClassMetadata, InitialStateTriggerType } from '@models/stateSpace';
...
import { InputParamField } from './editor/execution-panel/execution-panel.component';
import { TargetRuntime } from '@models/noCode/mock-NCS-data';
import { StateContextMenuAction, SIZE_PRESETS } from './editor/state-context-menu/state-context-menu.component';
import { StateConnectionInfo } from './popups/state-page-popup/state-page-popup.component';
import { SlotConfiguration, SlotConnectionInfo, InputMappingMode, OutputMappingMode, ProducedVariable } from './popups/slot-configuration-popup/slot-configuration-popup.component';
...
export class CustomNoCodeComponent implements OnInit, AfterViewInit, OnDestroy
{
  ...
  private updateBoundClassAndCodeWithInstances(solutionData: any, freshInstances: NoCodeState[]): void {
    ...
    // Use the live NoCodeSolution for code generation. We temporarily swap in the fresh
    // instances so the code generators see up-to-date connectors/field values, then restore
    // the original instances so overlays (which hold references) aren't disrupted.
    // IMPORTANT: Do NOT create a new NoCodeSolution here — its constructor subscribes to the
    // renderer manager's base layer and re-renders all D3 elements, destroying event handlers.
    const codeGenSolution = this.noCodeSolution;
    const originalInstances = codeGenSolution?.stateInstances;
    if (codeGenSolution) {
      codeGenSolution.stateInstances = freshInstances;
    }
    ...
    if (this.solutionTargetRuntime === 'typescript_frontend') {
      this.generatedTypescriptCode = this.typescriptCodeGenerator.generateFromSolution(
        codeGenSolution || null, this.boundClass, functionName
      );
    ...
    if (codeGenSolution && originalInstances) {
      codeGenSolution.stateInstances = originalInstances;
    }
