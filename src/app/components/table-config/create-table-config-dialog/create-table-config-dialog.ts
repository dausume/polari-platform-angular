...
export interface CreateTableConfigDialogData {
  name?: string;
  description?: string;
}
...
@Component({
  standalone: true,
  selector: 'create-table-config-dialog',
  template: `
    <h2 mat-dialog-title>Create Table Configuration</h2>
    <mat-dialog-content>
      <mat-form-field appearance="outline" class="full-width">
        <mat-label>Configuration Name</mat-label>
        <input matInput [(ngModel)]="name" placeholder="My Table Config" required>
      </mat-form-field>
      <mat-form-field appearance="outline" class="full-width">
...
export class CreateTableConfigDialogComponent {
  name: string = '';
  description: string = '';

  constructor(
    @Inject(MAT_DIALOG_DATA) public data: CreateTableConfigDialogData | null,
    private dialogRef: MatDialogRef<CreateTableConfigDialogComponent>
  ) {
    if (data) {
      this.name = data.name || '';
