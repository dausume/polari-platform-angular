...
import { FORM_VALIDATION_KEY, StateEvalResult } from './engine-types';
import { Context, resolveValueSourceConfig, safeResolveValue } from './value-sources';

export function handleFormValidation(
    fieldValues: any, context: Context, stateName: string, logOutput: string[],
    result: StateEvalResult,
): void {
    const fieldsCfg = (fieldValues.fields || []).filter(
        (f: any) => f && typeof f === 'object' && (f.enabled ?? true));
    const verdicts: Record<string, { valid: boolean; errors: string[] }> = {};
    ...
    for (const f of fieldsCfg) {
        ...
        if (errors.length) invalidFields.push(fname);
    }
    ...
    if (formValid) {
        ...
        logOutput.push(
            `[${stateName}] all ${fieldsCfg.length} field(s) valid `
        ...
        logOutput.push(
            `[${stateName}] validation FAILED for ${invalidFields.join(', ')} — `
...
export function handleValidationResult(
    ...
    const resolveFlexible = (raw: any) =>
        (raw && typeof raw === 'object' && 'sourceType' in raw)
            ? resolveValueSourceConfig(raw, context)
            : safeResolveValue(raw, context);
    ...
    for (const mapping of fieldValues.outputMappings || []) {
        ...
        if (outName) {
            context[outName] = resolveValueSourceConfig(mapping.valueSource, context);
            wrote.push(outName);
        }
    }
    result.result = 'outcome' in context ? context['outcome'] : context['complete'];
    logOutput.push(
        `[${stateName}] validation verdict bound: `
