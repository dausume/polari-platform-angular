...
export function resolveContextPath(path: string, context: Context): any {
    if (!path) return null;
    if (path in context) return context[path];
    if (path.startsWith('self.')) {
        const bare = path.slice(5);
        if (bare in context) return context[bare];
    }
    return null;
}

export function resolveValueSourceConfig(config: any, context: Context): any {
    ...
    if (sourceType === 'from_input') {
        ...
        if (varName && varName in context) return context[varName];
        return null;
    }
    ...
    if (sourceType === 'from_latex') {
        ...
        throw new Error(
            'This solution uses a LaTeX math source (from_latex), which '
    ...
    if (sourceType === 'array') {
        ...
        return elements.map((el: any) =>
            (typeof el === 'object' && el !== null && 'sourceType' in el)
    ...
    if (sourceType === 'element') {
        ...
        try {
            ...
            if (base === null || base === undefined || Number.isNaN(i)) return null;
            const v = base[i];
...
export function safeResolveValue(valueStr: any, context: Context): any {
    ...
    if (stripped in context) return context[stripped];
