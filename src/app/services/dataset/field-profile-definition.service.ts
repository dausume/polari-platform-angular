...
export class FieldProfileDefinitionService {

  configList$ = new BehaviorSubject<FieldProfileDefinitionSummary[]>([]);
  allConfigList$ = new BehaviorSubject<FieldProfileDefinitionSummary[]>([]);
  draftConfig$ = new BehaviorSubject<NamedFieldProfileConfig | null>(null);
  hasDraftChanges$ = new BehaviorSubject<boolean>(false);
  loading$ = new BehaviorSubject<boolean>(false);

  private readonly className = 'FieldProfileDefinition';
  ...
  fetchAllConfigs(): void {
    this.http.get<any>(this.baseUrl, this.polariService.backendRequestOptions).subscribe({
      next: (response: any) => {
        ...
        const summaries: FieldProfileDefinitionSummary[] = items.map((item: any) => ({
          id: item.id,
          name: item.name || '',
      ...
      error: (err: any) => {
        console.error('[FieldProfileDefinitionService] Failed to fetch all configs:', err);
      }
    });
  }
  ...
  fetchConfigsForClass(sourceClass: string): void {
    ...
    this.http.get<any>(this.baseUrl, this.polariService.backendRequestOptions).subscribe({
      next: (response: any) => {
        const items = this.parseReadAllResponse(response);
        const summaries: FieldProfileDefinitionSummary[] = items
      ...
      error: (err: any) => {
        console.error('[FieldProfileDefinitionService] Failed to fetch configs:', err);
        this.loading$.next(false);
      }
    });
  }

  loadConfig(id: string): Observable<NamedFieldProfileConfig> {
    this.loading$.next(true);
    return this.http.get<any>(this.baseUrl, this.polariService.backendRequestOptions).pipe(
      map((response: any) => {
        ...
        if (!backendObj) {
          throw new Error(`FieldProfileDefinition ${id} not found`);
        }
        return NamedFieldProfileConfig.fromBackend(backendObj);
      }),
      tap((config: NamedFieldProfileConfig) => {
  ...
  saveConfig(config: NamedFieldProfileConfig): Observable<any> {
  ...
  updateDraft(config: NamedFieldProfileConfig): void {
    this.draftConfig$.next(config);
    this.hasDraftChanges$.next(true);
  }
