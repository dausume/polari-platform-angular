...
export class GeoJsonDefinitionService {

    configList$ = new BehaviorSubject<GeoJsonDefinitionSummary[]>([]);
    draftConfig$ = new BehaviorSubject<NamedGeoJsonConfig | null>(null);
    hasDraftChanges$ = new BehaviorSubject<boolean>(false);
    loading$ = new BehaviorSubject<boolean>(false);

    private readonly className = 'GeoJsonDefinition';
    ...
    fetchConfigsForClass(sourceClass: string): void {
        ...
        this.http.get<any>(this.baseUrl, this.polariService.backendRequestOptions).subscribe({
            next: (response: any) => {
                ...
                const summaries: GeoJsonDefinitionSummary[] = items
            ...
            error: (err: any) => {
                console.error('[GeoJsonDefinitionService] Failed to fetch configs:', err);
    ...
    loadConfig(id: string): Observable<NamedGeoJsonConfig> {
        this.loading$.next(true);
        return this.http.get<any>(this.baseUrl, this.polariService.backendRequestOptions).pipe(
            map((response: any) => {
                ...
                if (!backendObj) {
                    throw new Error(`GeoJsonDefinition ${id} not found`);
                }
                return NamedGeoJsonConfig.fromBackend(backendObj);
            }),
            tap((config: NamedGeoJsonConfig) => {
    ...
    saveConfig(config: NamedGeoJsonConfig): Observable<any> {
    ...
    updateDraft(config: NamedGeoJsonConfig): void {
        this.draftConfig$.next(config);
        this.hasDraftChanges$.next(true);
    }
