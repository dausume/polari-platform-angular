 ...
 *   rather than merged — that file has its own, unrelated
 *   `EngineRootCapability` shape (originally both were named
 *   `EngineCapability`, a same-name different-shape collision; renamed
 *   2026-07-15 so a future consumer can't accidentally import the
 *   wrong one). Add new materials-basis response shapes here, not
 *   back in the service.
 * @see /OVERLAP_MAP.md
 */
...
/** Honest engine capability report (FEM + DFT ladders) — a
 *  deliberately narrowed view of the SAME `GET /api/msci/engines/
 *  capability` response `engine-model.service.ts` types more widely
 *  as `Record<string, EngineRootCapability>` (the real backend
 *  response — `scale_execution_api.py::on_get_capability` — always
 *  has fem/dft/md/meso keys; the materials-basis browser only reads
 *  fem/dft, so this type only names those two). If a consumer here
 *  ever needs md/meso too, import `EngineRootCapability` instead of
 *  widening this one. */
export interface EngineCapabilityLadders {
