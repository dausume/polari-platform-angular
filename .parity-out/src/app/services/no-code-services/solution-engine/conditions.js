...
exports.ARITHMETIC_OPS = exports.COMPARISON_OPS = void 0;
exports.evaluateCondition = evaluateCondition;
exports.evaluateChainLink = evaluateChainLink;
exports.evaluateConditionAny = evaluateConditionAny;
exports.evalElementExpression = evalElementExpression;
const value_sources_1 = require("./value-sources");
const eq = (a, b) => a === b || (typeof a === 'number' && typeof b === 'number' && a === b);
const contains = (a, b) => {
    if (typeof a === 'string')
...
exports.ARITHMETIC_OPS = {
    ...
    'modulo': (a, b) => exports.ARITHMETIC_OPS['%'](a, b),
...
function evaluateCondition(conditionData, context) {
    if (typeof conditionData === 'string') {
        return Boolean((0, value_sources_1.safeResolveValue)(conditionData, context));
    }
    ...
    const leftVal = (0, value_sources_1.safeResolveValue)(leftStr, context);
    const rightVal = (0, value_sources_1.safeResolveValue)(rightStr, context);
    const opFn = exports.COMPARISON_OPS[opStr] ?? eq;
    try {
...
function evaluateChainLink(link, context, logOutput) {
    const log = logOutput ?? [];
    const leftSource = link.leftSource;
    const leftVal = (leftSource && typeof leftSource === 'object' && 'sourceType' in leftSource)
        ? (0, value_sources_1.resolveValueSourceConfig)(leftSource, context)
        : (0, value_sources_1.safeResolveValue)(link.fieldName ?? '', context);
    const rightSource = link.rightSource;
    const rightVal = (rightSource && typeof rightSource === 'object' && 'sourceType' in rightSource)
        ? (0, value_sources_1.resolveValueSourceConfig)(rightSource, context)
        : (0, value_sources_1.safeResolveValue)(link.conditionValue ?? '', context);
    const conditionType = link.conditionType ?? 'equals';
    const opFn = exports.COMPARISON_OPS[conditionType] ?? eq;
    try {
...
function evaluateConditionAny(condition, context, logOutput) {
    ...
    if (typeof condition === 'string' && condition.trim()) {
        ...
        if (parts.length === 3 && parts[1] in exports.COMPARISON_OPS) {
            const left = (0, value_sources_1.safeResolveValue)(parts[0], context);
            const right = (0, value_sources_1.safeResolveValue)(parts[2], context);
            try {
                return Boolean(exports.COMPARISON_OPS[parts[1]](left, right));
            }
            catch {
                return false;
            }
        }
        return Boolean((0, value_sources_1.safeResolveValue)(condition, context));
    }
    return false;
}
/** Mirror of _eval_element_expression for Map/Reduce elements. */
function evalElementExpression(fieldValues, context, itemVar) {
    const valueSource = fieldValues.valueSource;
    if (valueSource && typeof valueSource === 'object' && 'sourceType' in valueSource) {
        return (0, value_sources_1.resolveValueSourceConfig)(valueSource, context);
    }
    const expression = String(fieldValues.expression ?? itemVar).trim();
    const parts = expression.split(/\s+/).filter(Boolean);
    if (parts.length === 3 && parts[1] in exports.ARITHMETIC_OPS) {
        const left = (0, value_sources_1.safeResolveValue)(parts[0], context);
        const right = (0, value_sources_1.safeResolveValue)(parts[2], context);
        try {
            return exports.ARITHMETIC_OPS[parts[1]](left, right);
        }
        catch {
            return null;
        }
    }
    return (0, value_sources_1.safeResolveValue)(expression, context);
}
