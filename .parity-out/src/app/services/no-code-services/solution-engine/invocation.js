...
exports.handleSolutionInvocation = handleSolutionInvocation;
exports.handleAwaitBackendCall = handleAwaitBackendCall;
const engine_types_1 = require("./engine-types");
const capability_1 = require("./capability");
const value_sources_1 = require("./value-sources");
function resolveInputMappings(fieldValues, context) {
    const childInputs = {};
    for (const m of fieldValues.inputMappings || []) {
        ...
        childInputs[param] = (src && typeof src === 'object')
            ? (0, value_sources_1.resolveValueSourceConfig)(src, context) : src;
    }
    return childInputs;
}
function validateContract(contract, childInputs, stateName, calleeName, logOutput) {
    ...
    if (missing.length) {
        ...
        throw new Error(`SolutionInvocation '${stateName}': solution `
    ...
    if (declaredInputs.length) {
        ...
        for (const extra of Object.keys(childInputs)) {
            if (!declaredNames.has(extra)) {
                logOutput.push(`[${stateName}] note: input '${extra}' is not in `
...
function bindResults(fieldValues, context, stateName, calleeName, logOutput, finalReturnValue, childOutputs) {
    for (const b of fieldValues.resultBindings || []) {
        ...
        if (output === 'return') {
            ...
            logOutput.push(`[${stateName}] note: '${calleeName}' produced no `
        ...
        logOutput.push(`[${stateName}] ${varName} = ${(0, engine_types_1.pyRepr)(context[varName])} (from ${output})`);
    }
}
/** Client mirror of _invoke_solution. */
async function handleSolutionInvocation(host, fieldValues, context, stateName, logOutput, result) {
    const calleeName = String(fieldValues.solutionRef || fieldValues.solutionName || '').trim();
    if (!calleeName) {
        throw new Error(`SolutionInvocation '${stateName}': no solutionRef `
            + `configured — pick which solution to invoke.`);
    }
    const chain = host.invocationChain;
    if (chain.length >= engine_types_1.MAX_INVOCATION_DEPTH) {
        const path = [...chain, host.solutionName, calleeName].join(' -> ');
        throw new Error(`SolutionInvocation '${stateName}': call nesting exceeded `
            + `${engine_types_1.MAX_INVOCATION_DEPTH} levels (${path}). If this is `
            + `recursion, make sure the base case is reachable; if the `
            + `nesting is intentional, flatten some of the chain.`);
    }
    if (!host.resolver) {
        throw new Error(`SolutionInvocation '${stateName}': no solution resolver is `
    ...
    const verdict = (0, capability_1.solutionRequiresBackend)(row.definition);
    if (verdict.backendRequired) {
        throw new Error(`SolutionInvocation '${stateName}': solution '${calleeName}' `
    ...
    logOutput.push(`[${stateName}] invoking '${calleeName}' `
    ...
    const childTrace = await host.executeChild(row, { ...childInputs }, [...chain, host.solutionName]);
    ...
    bindResults(fieldValues, context, stateName, calleeName, logOutput, childTrace.finalReturnValue, childOutputs);
    logOutput.push(`[${stateName}] '${calleeName}' completed in `
...
async function handleAwaitBackendCall(host, fieldValues, context, stateName, logOutput, result) {
    const calleeName = String(fieldValues.solutionRef || fieldValues.solutionName
        || fieldValues.backendSolution || '').trim();
    if (!calleeName) {
        throw new Error(`AwaitBackendCall '${stateName}': no solutionRef configured `
            + `— pick which backend solution to call.`);
    }
    if (!host.bridge) {
        throw new Error(`AwaitBackendCall '${stateName}': no backend connection is `
            + `available in this runtime.`);
    }
    const childInputs = resolveInputMappings(fieldValues, context);
    logOutput.push(`[${stateName}] awaiting backend solution '${calleeName}' with `
        + `[${Object.keys(childInputs).sort().map((k) => `'${k}'`).join(', ')}]`);
    const res = await host.bridge.invokeBackendSolution(calleeName, childInputs);
    if (res.status !== 'completed') {
        throw new Error(`AwaitBackendCall '${stateName}': backend solution `
            + `'${calleeName}' failed: ${res.error || 'unknown engine error'}`);
    }
    bindResults(fieldValues, context, stateName, calleeName, logOutput, res.finalReturnValue, res.outputs || {});
    const summary = {
        executionId: '(backend)',
        solutionName: calleeName,
        status: res.status,
